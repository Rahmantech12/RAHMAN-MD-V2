...
cmd({
    ...
    if (message.quoted.mtype !== 'stickerMessage') {
        return await client.sendMessage(from, {
            text: "❌ σηℓү sтιcкεя мεssαgεs cαη вε cσηvεятε∂"
        }, { quoted: message });
    }

    // Send processing message
    await client.sendMessage(from, {
        text: "🔄 ωαιт үσυя sтιcкεя cσηvεятιηg тσ ιмαgε.."
    }, { quoted: message });
    ...
    try {
        ...
        await client.sendMessage(from, {
            text: "❌ ρℓεαsε тяү ωιтн α ∂ιғғεяεηт sтιcкεя."
        }, { quoted: message });
    }
});
