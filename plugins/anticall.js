...
async (conn, mek, m, { isOwner, reply, from, sender, args, prefix }) => { // Added 'prefix' to destructured parameters
    try {
        ...
        if (arg === 'on') {
            if (currentStatus) {
                replyText = `📞 αηтι-cαℓℓ ғεαтυяε ιs αℓяεα∂ү *εηαвℓε∂*.`;
                finalReactionEmoji = 'ℹ️'; // Info reaction
            } else {
                settingsManager.setSetting('ANTICALL', true);
                replyText = `📞 αηтι-cαℓℓ ғεαтυяε нαs вεεη *εηαвℓε∂*!`;
                finalReactionEmoji = '✅'; // Enabled reaction
            }
        } else if (arg === 'off') {
            if (!currentStatus) {
                replyText = `📞 αηтι-cαℓℓ ғεαтυяε ιs αℓяεα∂ү *∂ιsαвℓε∂*.`;
                finalReactionEmoji = 'ℹ️'; // Info reaction
            } else {
                settingsManager.setSetting('ANTICALL', false);
                replyText = `📞 αηтι-cαℓℓ ғεαтυяε нαs вεεη *∂ιsαвℓε∂*!`;
        ...
        await conn.sendMessage(from, {
            ...
            contextInfo: {
                ...
                forwardedNewsletterMessageInfo: {
                    newsletterJid: '120363201214007503@newsletter', // Ensure this JID is valid
